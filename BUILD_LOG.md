...
> **Last updated:** 2026-06-20 · **Status:** core pipeline + model + backtest working end-to-end.
...
backtests them against the current system with kWh/€/CO₂ savings. Runs green from
`python scripts/train.py` and `python scripts/backtest.py`.
...
- [x] **Event-level backtest** — `backtest.py`: B1 (observed) vs B3 (model), per-event metrics + kWh/€/CO₂, dedupes the doubled heating events.
- [x] **Cross-window check** — `evaluate.py`: apply heating β to the cooling window.
- [x] **CLI + artifacts** — `scripts/train.py` → `models/rc_params.json`; `scripts/backtest.py` → `outputs/backtest_heating.csv`.
...
| Mean preheat lead (B3) | **4.07 h** (range 3.1–5.0) |
| On-time comfort | **B1 0/7 · B3 7/7** |
| Mean room temp at deadline | B1 18.5 °C · B3 20.5 °C |
| Preheat-window energy saved | **~558 kWh · ~71% · €167 · 223 kg CO₂** (7 mornings) |
...
- **Cooling backtest not done** — needs a symmetric precool controller.
...
- [ ] **Savings / visualization layer** (Pillar 3) — turn `backtest.py` output into the judge-facing
      view: per-event kWh/€/CO₂ bars, the 0/7→7/7 comfort story, what-if sliders. (Streamlit or notebook.)
- [ ] **Cooling-window backtest** — symmetric precool controller (signs flipped, cold supply) so the
      realistic multi-event lecture days are validated too.
...
outputs/       backtest_heating.csv · diagnostic_*.png
